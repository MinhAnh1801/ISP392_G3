...
public class EditFeedback extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        // Lấy danh sách câu hỏi và câu trả lời hiện tại từ cơ sở dữ liệu
        ...
        List<Feedback> feedbackAnswers = feedbackDAO.getFeedbackByLecturerId(lecturerId);

        // Đặt danh sách câu hỏi và câu trả lời vào request attribute
        request.setAttribute("feedbackQuestions", feedbackQuestions);
        request.setAttribute("feedbackAnswers", feedbackAnswers);
        request.setAttribute("lecturerId", lecturerId);

        // Chuyển đến trang feedbackQuestions.jsp
        RequestDispatcher dispatcher = request.getRequestDispatcher("/feedbackQuestions.jsp");
