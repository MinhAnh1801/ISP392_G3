...
public class TimetableDAO {
    ...
    private static final String UPDATE_CAPACITY
            = "UPDATE Schedule SET available_slot = available_slot - 1 where id = ?";

    public void addScheduleToTimetable(int studentId, int scheduleId) throws SQLException {
        try (Connection conn = new DBContext().getConnection(); PreparedStatement ps = conn.prepareStatement(INSERT_TIMETABLE)) {
    ...
    public void updateSchedule(int id) throws SQLException {
        try (Connection conn = new DBContext().getConnection(); PreparedStatement ps = conn.prepareStatement(UPDATE_CAPACITY)) {
    ...
    public void addPayment(int userId, double amount, String status, String paymentType) throws SQLException {
        try (Connection conn = new DBContext().getConnection(); PreparedStatement ps = conn.prepareStatement(INSERT_PAYMENT)) {
    ...
    public void addStu_Sub(int student_id, int subject_id, int class_id) throws SQLException {
        String query = " INSERT INTO Student_Subjects(student_id, subject_id,status,enrollment_date,class_id) VALUES (?,?,?,?,?)";
    ...
    public void addtoClass(int student_id, int class_id, int subject_id) throws SQLException {
        String query = " INSERT INTO StudentClass(student_id, class_id,subject_id) VALUES (?,?,?)";
        try (Connection conn = new DBContext().getConnection(); PreparedStatement ps = conn.prepareStatement(query)) {
            ps.setInt(1, student_id);
    ...
    public List<Timetable> getStudentTimetable(int studentId) throws SQLException {
        ...
        String query = "SELECT s.day_of_week, s.start_time, s.end_time,s.class_id, sub.id,\n"
        ...
        try (Connection conn = new DBContext().getConnection(); PreparedStatement ps = conn.prepareStatement(query)) {
            ...
            while (rs.next()) {
                ...
                timetableEntry.setClass_id(rs.getInt("class_id"));
                ...
                timetableEntry.setSubject_id(rs.getInt("id"));
    ...
    // Method to get the subject detail
    public Timetable getSubjectDetail(int subject_id) throws SQLException {
        Timetable timetableEntry = null;
        String query = "SELECT s.day_of_week, s.start_time, s.end_time,s.class_id, sub.id,sub.name,\n"
                + "                       sub.code AS subject_code, cl.name AS classroom_name,\n"
                + "					   lp.email,lp.full_name,c.class_name\n"
                + "                       FROM Timetable ss \n"
                + "                       JOIN Schedule s ON ss.schedule_id = s.id \n"
                + "                       JOIN Subjects sub ON s.subject_id = sub.id \n"
                + "                       JOIN Classrooms cl ON s.classroom_id = cl.id\n"
                + "					   Left JOIN Lecturer_Timetable lt on ss.schedule_id = lt.schedule_id\n"
                + "					   Left Join Lecturer_Profile lp on lt.lecturer_id = lp.lecturer_id\n"
        ...
        try (Connection conn = new DBContext().getConnection(); PreparedStatement ps = conn.prepareStatement(query)) {
            ...
            if (rs.next()) {
                ...
                timetableEntry.setClass_id(rs.getInt("class_id"));
                timetableEntry.setSubject_code(rs.getString("subject_code"));
                timetableEntry.setClassroomName(rs.getString("classroom_name"));
                timetableEntry.setClass_name(rs.getString("class_name"));
                timetableEntry.setLecturer_name(rs.getString("full_name"));
                timetableEntry.setSubjectName(rs.getString("name"));  
                timetableEntry.setLecturer_email(rs.getString("email"));
            }
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            System.out.println(dao.getSubjectDetail(11));
        } catch (Exception e) {
        }
