...
public class DashboardClassrooms extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (action) {
            ...
            case "detail":
                ...
                break;
            case "viewToUpdate":
                viewDetailsToUpdate(request, response);
                break;
    ...
    private void viewDetailsToUpdate(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        int classroomID = Integer.parseInt(request.getParameter("id"));
    ...
    private void updateFacilities(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        int classroomID = Integer.parseInt(request.getParameter("id"));

        // Lấy thông tin cơ sở vật chất từ form
        ...
        int totalChairs = Integer.parseInt(request.getParameter("totalChairs"));
        ...
        int totalLights = Integer.parseInt(request.getParameter("totalLights"));
        ...
        int totalProjectors = Integer.parseInt(request.getParameter("totalProjectors"));
        String projectorCondition = request.getParameter("projectorCondition");

        // Tạo đối tượng ClassroomFacilities để cập nhật
        ...
        facilities.setTotalTables(totalTables);
        facilities.setTableCondition(tableCondition);
        facilities.setNumberOfChairs(numberOfChairs);
        facilities.setTotalChairs(totalChairs);
        facilities.setChairCondition(chairCondition);
        facilities.setNumberOfLights(numberOfLights);
        facilities.setTotalLights(totalLights);
        facilities.setLightCondition(lightCondition);
        facilities.setNumberOfProjectors(numberOfProjectors);
        facilities.setTotalProjectors(totalProjectors);
        facilities.setProjectorCondition(projectorCondition);

        // Khởi tạo DAO và kiểm tra sự tồn tại của ClassroomFacilities với classroomID
        ClassroomFacilitiesDAO facilitiesDAO = new ClassroomFacilitiesDAO();
        ClassroomFacilities existingFacilities = facilitiesDAO.findFacilitiesByClassroomId(classroomID);

        if (existingFacilities != null) {
            // Nếu đã tồn tại, tiến hành cập nhật
            facilitiesDAO.update(facilities);
            request.getSession().setAttribute("successMessage", "Classroom facilities updated successfully!");
        } else {
            // Nếu chưa tồn tại, thực hiện thêm mới
            facilitiesDAO.insert(facilities);
            request.getSession().setAttribute("successMessage", "Classroom facilities added successfully!");
        }

        // Đặt thông báo thành công
        request.getSession().setAttribute("successMessage", "Classroom facilities updated successfully!");

        // Chuyển hướng về trang dashboardClassrooms
        response.sendRedirect("dashboardClassrooms");
    }
