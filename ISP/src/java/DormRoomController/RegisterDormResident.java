...
@Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int studentId = 2; // Giả định studentId là 2, bạn có thể sửa để lấy từ session hoặc request
        int dormRoomId;
   ...
   // DormResidentDAO dormResidentDAO = new DormResidentDAO();
    boolean paymentCreated = dormResidentDAO.addPayment(studentId, totalCost, "Fee");
    if (!paymentCreated) {
        ...
        // Kiểm tra nếu thanh toán đã được hoàn tất (status là 'Paid')
            ...
            response.sendRedirect("registerDormResident?errorMessage=Please go to the payment section to complete the procedure");
